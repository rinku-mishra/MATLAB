...
function [p2,X2] = surface_plot_w_pi_ratio(~)
clc;clear ;
ev = 1.6e-19;
e =  1.6e-19;
TiA = 0.05*ev;
...
%%%% Calculated values %%%%%
    ...
    for k = 1:t 
        ...
        b = (1/(w_piB^2))*((-(2*ks(k)^2*w_piA^2*lambda_eB^2*lambda_eA^2))-(w_piA^2*lambda_eB^2)+(2*ks(k)^2*(w_pd^2+w_piB^2)*lambda_eB^2*lambda_eA^2)+((w_pd^2+w_piB^2)*lambda_eA^2));
        c = (1/(w_piB^4))*((ks(k)^2*lambda_eA^2*lambda_eB^2*w_piA^4)-(lambda_eB^2*lambda_eA^2*ks(k)^2*((w_piB^2+w_pd^2)^2)));
